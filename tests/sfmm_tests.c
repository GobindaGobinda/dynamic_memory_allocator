...
Test(sf_memsuite_student, realloc_smaller_block_free_block, .init = sf_mem_init, .fini = sf_mem_fini) {
    void *x = sf_malloc(sizeof(double) * 8); // 16 + 64 = 80
    void *y = sf_realloc(x, sizeof(int));   //32
...
Test(sf_memsuite_student, create_multiple_free_list_same_segment, .init = sf_mem_init, .fini = sf_mem_fini) {


    void *x =  sf_malloc(PAGE_SZ /256);
    //void *y =
     sf_malloc(5);
    void *z = sf_malloc(PAGE_SZ /256);
    //void *a =
    sf_malloc(5);
    void *b = sf_malloc(PAGE_SZ /256);
    //void *c =
    sf_malloc(5);
    ...
    free_list *fl = &seg_free_list[find_list_index_from_size(PAGE_SZ/256 +16)];

    cr_assert_not_null(fl->head, "No block in expected free list");
    cr_assert_not_null(fl->head->next->next, "At least 2 Blocks expected in this list!");
    cr_assert(fl->head->next->header.block_size << 4 == 32);
    cr_assert(fl->head->header.allocated == 0);
    cr_assert(sf_errno == 0, "sf_errno is not zero!");
}
...
Test(sf_memsuite_student, pointer_validation_realloc, .init = sf_mem_init, .fini = sf_mem_fini, .signal = SIGABRT) {
    sf_errno = 0;

    void *x = sf_malloc(sizeof(int));
    sf_realloc((char*)x +1, 320);
}
...
Test(sf_memsuite_student, coalesece_check, .init = sf_mem_init, .fini = sf_mem_fini) {
    ...
    //backward coalesing should create one big block before sf_malloc(16)
    sf_free(b);
    sf_free(a);
    sf_free(y);
    sf_free(x);
     ...
     free_list *fl = &seg_free_list[find_list_index_from_size(32*4)];

     cr_assert_not_null(fl->head, "No block in expected free list");
     cr_assert(fl->head->header.block_size << 4 == (32*4));
     cr_assert(fl->head->header.allocated == 0);
     cr_assert(sf_errno == 0, "sf_errno is not zero!");
...
Test(sf_memsuite_student, padding_check, .init = sf_mem_init, .fini = sf_mem_fini) {


    char *x =  sf_malloc(64); // has no padding


    cr_assert( ((sf_free_header*)(x-8))->header.padded == 0);

    //char *b =
      x =  sf_realloc(x,12); // should have padding


    cr_assert( ((sf_free_header*)(x-8))->header.padded == 1);
